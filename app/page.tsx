...
export default function FitnessPlanner() {
  ...
  // --- DYNAMIC RESPONSIVE FONT SCALE & STYLING CLASSES (MOBILE-OPTIMIZED) ---
  const isLarge = settings.fontSize === 'large';
  const fontSizeClass = isLarge ? 'text-base sm:text-lg md:text-2xl' : 'text-sm sm:text-base md:text-lg';
  const headingClass = isLarge ? 'text-2xl sm:text-3xl md:text-5xl font-extrabold' : 'text-xl sm:text-2xl md:text-3xl font-bold';
  const subHeadingClass = isLarge ? 'text-lg sm:text-xl md:text-3xl font-bold' : 'text-base sm:text-lg md:text-2xl font-semibold';
  const bodyTextClass = isLarge ? 'text-base sm:text-lg md:text-2xl' : 'text-sm sm:text-base md:text-xl';
  
  const isDark = settings.highContrastMode;
  const bgClass = isDark ? 'bg-black text-yellow-300' : 'bg-slate-50 text-slate-900';
  const cardClass = isDark ? 'bg-zinc-950 border-2 sm:border-4 border-yellow-400 text-yellow-300' : 'bg-white border-2 sm:border-4 border-slate-300 text-slate-900';
  const subCardClass = isDark ? 'bg-zinc-900 border sm:border-2 border-yellow-500 text-white' : 'bg-slate-100 border sm:border-2 border-slate-300 text-slate-900';
  const btnClass = isDark ? 'bg-yellow-400 text-black border-2 sm:border-4 border-yellow-200 hover:bg-yellow-300' : 'bg-blue-700 text-white border-2 sm:border-4 border-blue-900 hover:bg-blue-800';
  const inputClass = isDark ? 'bg-zinc-900 text-yellow-300 border-2 sm:border-4 border-yellow-400' : 'bg-white text-slate-900 border-2 sm:border-4 border-slate-400';

  // --- VIEW: LOGIN ---
  if (!isLoggedIn && !isGuestMode) {
    return (
      <div className={`min-h-screen ${bgClass} flex flex-col items-center justify-between p-3 sm:p-6 font-sans`}>
        <div className="max-w-2xl w-full space-y-4 sm:space-y-8 my-auto">
          <h1 className={`${headingClass} text-center border-b-4 sm:border-b-8 border-current pb-4 sm:pb-6`}>
            Senior Health Station
          </h1>
          
          <div className={`${cardClass} p-4 sm:p-8 md:p-10 rounded-2xl shadow-xl space-y-4 sm:space-y-8`}>
            ...
            <form onSubmit={handleSignIn} className="space-y-4 sm:space-y-6">
              ...
              <input 
                ...
                className={`w-full px-4 sm:px-6 py-3 sm:py-6 ${bodyTextClass} rounded-xl ${inputClass}`}
              />
              <button type="submit" className={`w-full font-extrabold ${subHeadingClass} py-4 sm:py-6 rounded-xl ${btnClass}`}>
          ...
          <div className="text-center pt-2 sm:pt-4">
            <p className={`${subHeadingClass} mb-3 sm:mb-6 opacity-80`}>Or use the app without an account:</p>
            <button onClick={handleGuest} className={`w-full font-extrabold ${subHeadingClass} py-4 sm:py-6 rounded-xl shadow-md ${subCardClass}`}>
  ...
  // --- VIEW: MAIN SHELL ---
  return (
    <div className={`min-h-screen p-3 sm:p-6 md:p-8 font-sans ${fontSizeClass} ${bgClass} flex flex-col justify-between`}>
      <div className="max-w-4xl mx-auto w-full space-y-4 sm:space-y-8">
        ...
        <header className={`${cardClass} p-4 sm:p-6 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-6 shadow-sm print:hidden`}>
          <div>
            <h1 className={headingClass}>Health Station</h1>
            <p className={`${bodyTextClass} mt-1 sm:mt-2 font-medium opacity-90`}>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 w-full sm:w-auto justify-end">
            ...
            <button 
              ...
              className={`bg-red-700 text-white font-extrabold px-4 sm:px-6 py-2.5 sm:py-4 rounded-xl border-2 sm:border-4 border-red-950 hover:bg-red-800 flex items-center justify-center gap-2 shadow-lg w-full sm:w-auto ${bodyTextClass}`}
            >
              🚨 Emergency Contacts
            </button>

            {currentScreen !== 'menu' && (
              <button onClick={() => setCurrentScreen('menu')} className={`font-bold px-4 sm:px-6 py-2.5 sm:py-4 rounded-xl ${bodyTextClass} ${subCardClass} w-full sm:w-auto`}>
                🔙 Menu
              </button>
            )}
          </div>
        ...
        {showEmergencyModal && (
          <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-3 sm:p-4">
            <div className={`max-w-2xl w-full p-4 sm:p-8 rounded-2xl border-4 sm:border-8 border-red-600 ${isDark ? 'bg-zinc-950 text-white' : 'bg-white text-slate-900'} space-y-4 sm:space-y-6 max-h-[90vh] overflow-y-auto`}>
              <div className="flex justify-between items-center border-b-2 sm:border-b-4 border-red-600 pb-3 sm:pb-4">
                <h3 className={`${headingClass} text-red-600`}>🚨 Emergency Quick Actions</h3>
                <button onClick={() => setShowEmergencyModal(false)} className="text-xl sm:text-3xl font-bold px-3 sm:px-4 py-1 sm:py-2 bg-slate-200 text-black rounded-lg">✕</button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <a 
                  href="tel:911" 
                  className={`block text-center bg-red-700 text-white font-extrabold ${subHeadingClass} py-4 sm:py-6 rounded-2xl border-2 sm:border-4 border-red-950 hover:bg-red-800`}
                >
                  📞 Call 911 Directly
                </a>
                ...
                <a 
                  href="sms:911" 
                  className={`block text-center bg-zinc-800 text-white font-extrabold ${subHeadingClass} py-4 sm:py-6 rounded-2xl border-2 sm:border-4 border-black hover:bg-zinc-900`}
                >
                  💬 Text 911 (SMS)
                </a>
              </div>
              ...
              {settings.emergencyContactPhone ? (
                <a 
                  ...
                  className={`block w-full text-center bg-amber-600 text-white font-extrabold ${subHeadingClass} py-4 sm:py-6 rounded-2xl border-2 sm:border-4 border-amber-900 hover:bg-amber-700`}
                >
                  📞 Call Personal Contact: {settings.emergencyContactName || 'Caregiver'} ({settings.emergencyContactPhone})
                </a>
              ) : (
                <div className={`p-3 sm:p-4 rounded-xl border-2 sm:border-4 border-amber-500 bg-amber-50 text-amber-900 text-center space-y-2 sm:space-y-3`}>
                  ...
                  <button 
                    ...
                    className="bg-amber-600 text-white font-extrabold px-4 sm:px-6 py-2 sm:py-3 rounded-xl hover:bg-amber-700"
                  >
              ...
              {settings.emergencyContact2Phone && (
                <a 
                  ...
                  className={`block w-full text-center bg-teal-700 text-white font-extrabold ${subHeadingClass} py-4 sm:py-6 rounded-2xl border-2 sm:border-4 border-teal-950 hover:bg-teal-800`}
                >
              ...
              <button 
                ...
                className={`block w-full text-center bg-blue-700 text-white font-extrabold ${subHeadingClass} py-4 sm:py-5 rounded-2xl border-2 sm:border-4 border-blue-900 hover:bg-blue-800`}
              >
              ...
              {settings.emergencyWebUrl && (
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className={`block w-full text-center bg-slate-700 text-white font-bold ${bodyTextClass} py-3 sm:py-4 rounded-2xl border-2 sm:border-4 border-slate-900 hover:bg-slate-800`}
                >
                  🌐 External Healthcare Web Portal ↗
                </a>
              )}

              <button onClick={() => setShowEmergencyModal(false)} className={`w-full bg-slate-800 text-white font-bold ${bodyTextClass} py-3 sm:py-4 rounded-xl`}>
        ...
        {showMedicalCardModal && (
          <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-3 sm:p-4">
            <div className={`max-w-2xl w-full p-4 sm:p-8 rounded-2xl border-4 sm:border-8 border-blue-600 ${isDark ? 'bg-zinc-950 text-white' : 'bg-white text-slate-900'} space-y-4 sm:space-y-6 max-h-[90vh] overflow-y-auto`}>
              <div className="flex justify-between items-center border-b-2 sm:border-b-4 border-blue-600 pb-3 sm:pb-4">
                <h3 className={`${headingClass} text-blue-600`}>🪪 Emergency Medical Profile</h3>
                <button onClick={() => setShowMedicalCardModal(false)} className="text-xl sm:text-3xl font-bold px-3 sm:px-4 py-1 sm:py-2 bg-slate-200 text-black rounded-lg">✕</button>
              </div>

              <div className={`${subCardClass} p-4 sm:p-6 rounded-xl space-y-3 sm:space-y-4`}>
                ...
                <div className="border-t border-current pt-3 sm:pt-4">
                  <p className={`${subHeadingClass} mb-1 sm:mb-2`}>Medical Notes & Conditions:</p>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <button onClick={() => { setShowMedicalCardModal(false); setCurrentScreen('settings'); }} className={`flex-1 bg-amber-600 text-white font-bold ${bodyTextClass} py-3 sm:py-4 rounded-xl`}>
                  ✏️ Edit Profile & Contacts
                </button>
                <button onClick={() => setShowMedicalCardModal(false)} className={`flex-1 bg-slate-800 text-white font-bold ${bodyTextClass} py-3 sm:py-4 rounded-xl`}>
                  Close Profile
                </button>
              </div>
            </div>
        ...
        {currentScreen === 'menu' && (
          <div className="space-y-4 sm:space-y-8">
            <div className={`${cardClass} p-4 sm:p-8 rounded-2xl`}>
              <h2 className={`${subHeadingClass} mb-3 sm:mb-6`}>Step 1: Choose a Date</h2>
              <div className="flex overflow-x-auto gap-2 sm:gap-4 pb-2 sm:pb-4">
                {calendarDays.map(day => (
                  <button
                    key={day.dateString}
                    onClick={() => setSelectedDate(day.dateString)}
                    className={`min-w-[100px] sm:min-w-[140px] p-3 sm:p-6 border-2 sm:border-4 rounded-2xl flex flex-col items-center justify-center transition-all ${
                  ...
                  >
                    ...
                    <span className={`${subHeadingClass} mt-1 sm:mt-2`}>{day.dayOfMonth}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* UN & CDC Quick Banner */}
            <button onClick={() => setCurrentScreen('sdgInfo')} className={`w-full p-4 sm:p-6 rounded-2xl border-2 sm:border-4 text-left flex justify-between items-center ${isDark ? 'border-yellow-400 bg-zinc-900' : 'border-blue-600 bg-blue-50'}`}>
              <div>
                <span className="text-xs sm:text-sm uppercase tracking-wider font-extrabold text-blue-600 dark:text-yellow-400">CDC & UN SDG 3 Alignment</span>
                <h3 className={`${subHeadingClass} mt-1`}>Healthy Aging & Fall Prevention Standards</h3>
              </div>
              <span className={`${subHeadingClass} hidden sm:inline`}> UN SDG Goals / CDC Guidelines ➔</span>
            </button>

            <h2 className={`${subHeadingClass} pt-2`}>Step 2: What would you like to do?</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6">
              <button onClick={() => setCurrentScreen('exercises')} className={`p-5 sm:p-8 md:p-10 rounded-2xl text-left border-2 sm:border-4 ${isDark ? 'border-yellow-400 bg-zinc-900' : 'bg-blue-50 border-blue-600 hover:bg-blue-100'}`}>
                <h3 className={`${subHeadingClass} mb-1 sm:mb-2`}>🏋️ Exercises</h3>
                <p className={`${bodyTextClass} opacity-90`}>CDC-guided routines & active timers.</p>
              </button>
              
              <button onClick={() => setCurrentScreen('progress')} className={`p-5 sm:p-8 md:p-10 rounded-2xl text-left border-2 sm:border-4 ${isDark ? 'border-yellow-400 bg-zinc-900' : 'bg-green-50 border-green-600 hover:bg-green-100'}`}>
                <h3 className={`${subHeadingClass} mb-1 sm:mb-2`}>📊 My Progress</h3>
                <p className={`${bodyTextClass} opacity-90`}>Track CDC targets & UN SDG 3 status.</p>
              </button>

              <button onClick={() => setCurrentScreen('notes')} className={`p-5 sm:p-8 md:p-10 rounded-2xl text-left border-2 sm:border-4 ${isDark ? 'border-yellow-400 bg-zinc-900' : 'bg-yellow-50 border-yellow-600 hover:bg-yellow-100'}`}>
                <h3 className={`${subHeadingClass} mb-1 sm:mb-2`}>📝 Daily Notes</h3>
                <p className={`${bodyTextClass} opacity-90`}>Track hydration & wellness state.</p>
              </button>

              <button onClick={() => setCurrentScreen('settings')} className={`p-5 sm:p-8 md:p-10 rounded-2xl text-left border-2 sm:border-4 ${subCardClass}`}>
                <h3 className={`${subHeadingClass} mb-1 sm:mb-2`}>⚙️ Settings</h3>
                <p className={`${bodyTextClass} opacity-90`}>Text size, voice prompts & emergency contacts.</p>
              </button>
            </div>

            <div className="pt-2 sm:pt-6">
              <button onClick={handleLogOut} className={`w-full bg-red-700 text-white border-2 sm:border-4 border-red-900 p-4 sm:p-8 rounded-2xl ${subHeadingClass} hover:bg-red-800`}>
        ...
        {currentScreen === 'exercises' && (
          <div className={`${cardClass} p-4 sm:p-8 rounded-2xl space-y-6 sm:space-y-10`}>
            <div>
              <h2 className={`${headingClass} mb-3 sm:mb-6`}>Select Muscle Groups</h2>
              <p className={`${bodyTextClass} mb-4 sm:mb-6 opacity-80`}>Click target areas for today's session:</p>
                ...
                {MUSCLE_GROUPS.map(mName => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-4 sm:px-8 py-3 sm:py-6 ${subHeadingClass} border-2 sm:border-4 rounded-2xl ${chosen ? btnClass : subCardClass}`}
                    >
                      {chosen ? `✅ ${mName}` : mName}
                    </button>
                  );
            ...
            <div className="border-t-2 sm:border-t-4 border-current pt-6 sm:pt-10 space-y-6 sm:space-y-8">
              ...
              {EXERCISES_DB.filter(ex => selectedMuscles.length === 0 || ex.targetMuscles.some(m => selectedMuscles.includes(m))).map(ex => (
                <div key={ex.id} className={`${subCardClass} p-4 sm:p-8 rounded-2xl`}>
                  ...
                  <p className={`${bodyTextClass} mb-3 sm:mb-6 font-medium`}>{ex.description}</p>
                  <p className={`${bodyTextClass} font-bold mb-4 sm:mb-8 opacity-90`}>
                    Target Time: {ex.minutes} Minutes
                  </p>
                  <button onClick={() => startExerciseFlow(ex)} className={`w-full ${btnClass} ${subHeadingClass} py-4 sm:py-8 rounded-2xl`}>
        ...
        {currentScreen === 'timer' && timerExercise && (
          <div className={`${cardClass} p-4 sm:p-8 rounded-2xl space-y-6 sm:space-y-10 relative`}>
            {/* FLOATING EMERGENCY ASSISTANCE BUTTON */}
            <div className="sticky top-2 sm:top-4 z-40 flex justify-end">
              <button 
                onClick={() => setShowEmergencyModal(true)} 
                className={`bg-red-600 text-white font-extrabold px-4 sm:px-8 py-2.5 sm:py-4 rounded-full ${bodyTextClass} border-2 sm:border-4 border-red-950 shadow-2xl hover:bg-red-700 transition-transform active:scale-95 animate-pulse`}
              >
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-2 sm:gap-4 text-center">
              <span className={`px-4 sm:px-6 py-2 sm:py-3 rounded-xl border-2 sm:border-4 font-extrabold ${bodyTextClass} ${timerPhase === 'warmup' ? 'bg-yellow-400 text-black border-yellow-600' : 'opacity-40 border-current'}`}>
                1. Dynamic Warm-Up (2 min)
              </span>
              <span className={`px-4 sm:px-6 py-2 sm:py-3 rounded-xl border-2 sm:border-4 font-extrabold ${bodyTextClass} ${timerPhase === 'active' ? 'bg-blue-600 text-white border-blue-900' : 'opacity-40 border-current'}`}>
                2. Main Routine
              </span>
              <span className={`px-4 sm:px-6 py-2 sm:py-3 rounded-xl border-2 sm:border-4 font-extrabold ${bodyTextClass} ${timerPhase === 'cooldown' ? 'bg-green-600 text-white border-green-900' : 'opacity-40 border-current'}`}>
                3. Cool Down (2 min)
              </span>
            </div>
            
            <div className={`p-4 sm:p-8 rounded-2xl space-y-4 sm:space-y-6 border-4 sm:border-8 ${isDark ? 'border-yellow-400 bg-zinc-900' : 'border-yellow-500 bg-yellow-50'}`}>
              ...
              <div className="space-y-3 sm:space-y-4 pt-2 sm:pt-4">
                <label className={`flex items-center gap-3 sm:gap-6 p-4 sm:p-6 rounded-xl border-2 sm:border-4 cursor-pointer ${subCardClass}`}>
                  <input type="checkbox" className="w-8 h-8 sm:w-12 sm:h-12 accent-yellow-500 shrink-0" checked={safetyCleared.clearSpace} onChange={e => setSafetyCleared(p => ({...p, clearSpace: e.target.checked}))} />
                  <span className={`${bodyTextClass} font-bold`}>Floor is clear of tripping hazards.</span>
                </label>
                <label className={`flex items-center gap-3 sm:gap-6 p-4 sm:p-6 rounded-xl border-2 sm:border-4 cursor-pointer ${subCardClass}`}>
                  <input type="checkbox" className="w-8 h-8 sm:w-12 sm:h-12 accent-yellow-500 shrink-0" checked={safetyCleared.hasWater} onChange={e => setSafetyCleared(p => ({...p, hasWater: e.target.checked}))} />
                  <span className={`${bodyTextClass} font-bold`}>A glass of water is within reach.</span>
                </label>
                <label className={`flex items-center gap-3 sm:gap-6 p-4 sm:p-6 rounded-xl border-2 sm:border-4 cursor-pointer ${subCardClass}`}>
                  <input type="checkbox" className="w-8 h-8 sm:w-12 sm:h-12 accent-yellow-500 shrink-0" checked={safetyCleared.goodShoes} onChange={e => setSafetyCleared(p => ({...p, goodShoes: e.target.checked}))} />
            ...
            <div className={`p-4 sm:p-8 rounded-2xl border-2 sm:border-4 space-y-4 sm:space-y-6 ${subCardClass}`}>
              <h3 className={subHeadingClass}>CDC Intensity & Effort Level</h3>
              
              <div>
                <label className={`block ${bodyTextClass} font-bold mb-2 sm:mb-3`}>CDC Intensity Selection:</label>
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                  <button 
                    onClick={() => setCurrentIntensity('moderate')} 
                    className={`flex-1 py-3 sm:py-4 ${bodyTextClass} font-bold rounded-xl border-2 sm:border-4 ${currentIntensity === 'moderate' ? btnClass : 'bg-transparent border-current'}`}
                  >
                    🚶 Moderate (1x CDC Credit)
                  </button>
                  <button 
                    onClick={() => setCurrentIntensity('vigorous')} 
                    className={`flex-1 py-3 sm:py-4 ${bodyTextClass} font-bold rounded-xl border-2 sm:border-4 ${currentIntensity === 'vigorous' ? btnClass : 'bg-transparent border-current'}`}
                  >
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full h-6 sm:h-8 bg-slate-300 rounded-lg cursor-pointer"
                />
                <p className={`${bodyTextClass} opacity-80 mt-1`}>1 = Very Easy, 5 = Moderate Effort, 10 = Maximum Exertion</p>
              </div>
            </div>

            <div className={`text-center p-6 sm:p-10 rounded-2xl border-2 sm:border-4 ${subCardClass}`}>
              <div className="text-5xl sm:text-7xl md:text-8xl font-black mb-6 sm:mb-10 tracking-wider">
              ...
              <button 
                ...
                className={`w-full py-4 sm:py-8 ${headingClass} rounded-2xl border-4 sm:border-8 ${
                  timerActive 
            ...
            <div className={`p-4 sm:p-8 rounded-2xl border-2 sm:border-4 ${subCardClass}`}>
              <h3 className={`${subHeadingClass} mb-4 sm:mb-6`}>Instructions:</h3>
              <ul className="space-y-4 sm:space-y-6">
                {timerExercise.instructions.map((step, idx) => (
                  <li key={idx} className="flex gap-4 sm:gap-6 items-start">
                    <span className={`bg-current text-zinc-900 dark:text-black font-black rounded-full w-8 h-8 sm:w-12 sm:h-12 flex items-center justify-center shrink-0 ${bodyTextClass}`}>{idx + 1}</span>
        ...
        {currentScreen === 'progress' && (
          <div className={`${cardClass} p-4 sm:p-8 rounded-2xl space-y-6 sm:space-y-12 print:border-none print:shadow-none`}>
            <div>
              <h2 className={`${headingClass} mb-2 sm:mb-4`}>CDC & UN Goal Dashboard</h2>
              <p className={`${bodyTextClass} opacity-80 mb-4 sm:mb-8`}>Aligned with CDC guidelines for Adults 65+ and UN SDG Target 3.4 for Healthy Aging.</p>
              
              <div className="space-y-4 sm:space-y-8">
                <div className={`${subCardClass} p-4 sm:p-8 rounded-2xl`}>
                  <h3 className={`${subHeadingClass} mb-2`}>1. Aerobic Activity (CDC Target)</h3>
                  <p className={`${bodyTextClass} mb-2 sm:mb-4`}>Goal: At least 150 Moderate Minutes / week (Vigorous minutes count double)</p>
                  <p className={`${bodyTextClass} mb-2 sm:mb-4 font-bold`}>Weighted CDC Credit Logged: {calculateCdcAerobicCredit()} / 150 minutes</p>
                  <div className="w-full bg-zinc-300 dark:bg-zinc-700 h-6 sm:h-10 rounded-full border-2 sm:border-4 border-current overflow-hidden">
                ...
                <div className={`${subCardClass} p-4 sm:p-8 rounded-2xl`}>
                  ...
                  <p className={`${bodyTextClass} mb-1 sm:mb-2`}>Goal: At least 2 days / week</p>
                ...
                <div className={`${subCardClass} p-4 sm:p-8 rounded-2xl`}>
                  ...
                  <p className={`${bodyTextClass} mb-1 sm:mb-2`}>Goal: 3 days / week to reduce fall risks</p>
                  <p className={`${bodyTextClass} font-bold`}>Logged: {countDaysByCategory('balance')} unique days completed</p>
                </div>
              </div>
            </div>

            <div className="border-t-2 sm:border-t-4 border-current pt-6 sm:pt-10">
              <div className="flex flex-col sm:flex-row justify-between items-center mb-4 sm:mb-8 gap-3 sm:gap-4 print:hidden">
                <h2 className={headingClass}>Logs for {selectedDate}</h2>
                <div className="flex gap-2 sm:gap-4 w-full sm:w-auto">
                  <button onClick={printDoctorSummary} className={`font-bold ${bodyTextClass} px-4 sm:px-6 py-2.5 sm:py-4 rounded-xl border-2 sm:border-4 ${subCardClass} flex-1 sm:flex-none`}>
                    🖨️ Print Doctor Report
                  </button>
                  <button onClick={downloadMyLogsData} className={`font-bold ${bodyTextClass} px-4 sm:px-8 py-2.5 sm:py-4 rounded-xl border-2 sm:border-4 ${btnClass} flex-1 sm:flex-none`}>
                    📥 Download CSV
                  </button>
                </div>
              </div>
              ...
              {logs.filter(l => l.date === selectedDate).length === 0 ? (
                <p className={`${subCardClass} p-6 sm:p-10 border-dashed rounded-2xl text-center font-bold ${bodyTextClass}`}>
                  No activities logged for this date.
                </p>
              ) : (
                <div className="space-y-3 sm:space-y-4">
                  {logs.filter(l => l.date === selectedDate).map(log => (
                    <div key={log.id} className={`${subCardClass} p-4 sm:p-8 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-6`}>
                      <div className="text-center sm:text-left">
                        <h4 className={`${subHeadingClass} mb-1 sm:mb-2`}>{log.name}</h4>
                      ...
                      <button onClick={() => setLogs(prev => prev.filter(item => item.id !== log.id))} className={`bg-red-700 text-white border-2 sm:border-4 border-red-900 font-extrabold ${bodyTextClass} px-6 sm:px-8 py-2.5 sm:py-4 rounded-xl w-full sm:w-auto print:hidden`}>
                        Delete
                      </button>
                    </div>
                  ))}
        ...
        {currentScreen === 'notes' && (
          <div className={`${cardClass} p-4 sm:p-8 rounded-2xl space-y-6 sm:space-y-10`}>
            ...
            <div className="space-y-3 sm:space-y-6">
              <label className={`flex items-center gap-4 sm:gap-6 p-4 sm:p-8 rounded-2xl border-2 sm:border-4 cursor-pointer ${subCardClass}`}>
                <input type="checkbox" className="w-8 h-8 sm:w-12 sm:h-12 accent-blue-600 shrink-0" checked={currentFeltGood} onChange={e => setCurrentFeltGood(e.target.checked)} />
                <span className={`${subHeadingClass}`}>I felt energetic and active today.</span>
              </label>
              
              <label className={`flex items-center gap-4 sm:gap-6 p-4 sm:p-8 rounded-2xl border-2 sm:border-4 cursor-pointer ${subCardClass}`}>
                <input type="checkbox" className="w-8 h-8 sm:w-12 sm:h-12 accent-blue-600 shrink-0" checked={currentDrankWater} onChange={e => setCurrentDrankWater(e.target.checked)} />
                <span className={`${subHeadingClass}`}>I maintained adequate hydration.</span>
              </label>
            </div>

            <div className={`p-4 sm:p-8 rounded-2xl border-2 sm:border-4 space-y-3 sm:space-y-4 ${subCardClass}`}>
            ...
            <div className="space-y-3 sm:space-y-6">
              ...
              <textarea
                ...
                className={`w-full p-4 sm:p-6 ${bodyTextClass} rounded-2xl ${inputClass}`}
                placeholder="Log physical sensations, joint comfort, or personal wins..."
              />
            </div>

            <button onClick={saveNotes} className={`w-full ${btnClass} font-extrabold ${subHeadingClass} py-4 sm:py-8 rounded-2xl shadow-lg`}>
              Save Daily Notes
            </button>
          </div>
        )}
        ...
        {currentScreen === 'sdgInfo' && (
          <div className={`${cardClass} p-4 sm:p-8 rounded-2xl space-y-4 sm:space-y-8`}>
            <h2 className={headingClass}>United Nations SDG 3 & CDC Standards</h2>
            <div className={`${subCardClass} p-4 sm:p-6 rounded-xl space-y-2 sm:space-y-4`}>
            ...
            <div className={`${subCardClass} p-4 sm:p-6 rounded-xl space-y-2 sm:space-y-4`}>
              ...
              <ul className={`list-disc pl-6 sm:pl-8 ${bodyTextClass} space-y-1 sm:space-y-2`}>
        ...
        {currentScreen === 'settings' && (
          <div className={`${cardClass} p-4 sm:p-8 rounded-2xl space-y-6 sm:space-y-10`}>
            <h2 className={`${headingClass} border-b-2 sm:border-b-4 border-current pb-4 sm:pb-6`}>Emergency & App Settings</h2>
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="p-4 sm:p-6 rounded-2xl border-2 sm:border-4 border-red-500 bg-red-50 text-red-950 font-bold space-y-1 sm:space-y-2">
              ...
              <div className="space-y-4 sm:space-y-6">
                <div className="p-4 sm:p-6 rounded-xl border-2 sm:border-4 border-amber-500 space-y-3 sm:space-y-4">
                  <h4 className={`${subHeadingClass}`}>Primary Personal Contact</h4>
                  <div>
                    <label className={`block ${bodyTextClass} font-bold mb-1 sm:mb-2`}>Caregiver / Doctor Name:</label>
                    <input 
                      type="text" 
                      value={settings.emergencyContactName}
                      onChange={e => setSettings(p => ({...p, emergencyContactName: e.target.value}))}
                      className={`w-full p-3 sm:p-4 ${bodyTextClass} rounded-xl ${inputClass}`}
                      placeholder="e.g. Dr. Smith / Daughter Jane"
                    />
                  </div>
                  <div>
                    <label className={`block ${bodyTextClass} font-bold mb-1 sm:mb-2`}>Phone Number:</label>
                    <input 
                      type="tel" 
                ...
                <div className="p-4 sm:p-6 rounded-xl border-2 sm:border-4 border-teal-500 space-y-3 sm:space-y-4">
                  <h4 className={`${subHeadingClass}`}>Secondary Personal Contact (Optional)</h4>
                  <div>
                    <label className={`block ${bodyTextClass} font-bold mb-1 sm:mb-2`}>Family Member / Alternate Contact Name:</label>
                    <input 
                      type="text" 
                ...
                <div>
                  <label className={`block ${bodyTextClass} font-bold mb-1 sm:mb-2`}>Local Emergency Medical ID / Notes (No Signup):</label>
                  <textarea 
                    rows={3}
                  ...
                  />
                  <p className="text-xs sm:text-base opacity-80 mt-1">Stored locally on your device. Can be presented immediately to first responders without signing up for external websites.</p>
                </div>
                <div>
                  <label className={`block ${bodyTextClass} font-bold mb-1 sm:mb-2`}>Optional Telehealth / Medical Website URL:</label>
                  <input 
                    type="url" 
            ...
            <div className="border-t-2 sm:border-t-4 border-current pt-6 sm:pt-10 space-y-4 sm:space-y-6">
              <h3 className={subHeadingClass}>Display & Accessibility Settings</h3>
              
              <div className={`p-4 sm:p-8 rounded-2xl border-2 sm:border-4 space-y-2 sm:space-y-4 ${subCardClass}`}>
                ...
                <div className="flex gap-2 sm:gap-4">
                  <button 
                    ...
                    className={`flex-1 py-3 sm:py-4 ${bodyTextClass} font-bold rounded-xl border-2 sm:border-4 ${settings.fontSize === 'normal' ? btnClass : subCardClass}`}
                  >
                    Standard Size
                  </button>
                  <button 
                    onClick={() => setSettings(p => ({...p, fontSize: 'large'}))}
                    className={`flex-1 py-3 sm:py-4 ${bodyTextClass} font-bold rounded-xl border-2 sm:border-4 ${settings.fontSize === 'large' ? btnClass : subCardClass}`}
                  >
                    Large Print
                  </button>
                </div>
              ...
              <label className={`flex items-center gap-4 sm:gap-6 p-4 sm:p-8 rounded-2xl border-2 sm:border-4 cursor-pointer ${subCardClass}`}>
                <input 
                  type="checkbox" 
                  className="w-8 h-8 sm:w-12 sm:h-12 accent-yellow-400 shrink-0"
            ...
            <button onClick={() => setCurrentScreen('menu')} className={`w-full ${btnClass} font-extrabold ${headingClass} py-4 sm:py-8 rounded-2xl shadow-lg mt-4 sm:mt-8`}>
